...
class Cf_inbox extends CI_Controller{
  ...
  function cek_inbox()
  {
    $username=$this->uri->segment(3);
      $listdata= $this->MF_inbox->count_inbox($username)->num_rows();
      if ($listdata>0) {
        echo "<a href='".base_url('Cf_inbox/list_inbox')."'>Inbox<span class='badge badge-danger'> ".$listdata."</span></a>";
      }
      else{
        echo "<a href='".base_url('Cf_inbox/list_inbox')."'>Inbox</span></a>";
      }
  ...
  public function list_inbox()
  {
    ...
    $listdata_new= $this->MF_inbox->list_inbox_new($username)->result_array();
    // $listdata_terbaca= $this->MF_inbox->list_inbox_terbaca($username)->result_array();
    // print_r($listdata);
  ...
  public function list_inbox_ajax()
  {
    $load=$this->load;
    $username=$this->session->userdata('User');
    $daftar_list_new= $this->MF_inbox->list_inbox_new($username)->result_array();
    //$listdata_terbaca= $this->MF_inbox->list_inbox_terbaca($username)->result_array();
    // print_r($listdata);
  ...
  public function detail_inbox_ajax()
  {
    ...
    $dataupdtelist = array('status_terbaca_custumer' => 'terbaca', );
    $where = array('id_list_inbox' => $iddetail,);
    $daftar_list_detail= $this->MF_inbox->detail_inbox($iddetail)->result_array();
    $upadtesatatuslist=$this->MF_inbox->updatelist($where,$dataupdtelist);
    $upadtesatatusdetailist=$this->MF_inbox->updatedetaillist($where,$dataupdtelist);

    foreach ($daftar_list_detail as $row_detail) {
      if ($row_detail['status_pengirim']==1) {
        echo '<div class=""><p class="text-white  text-right font-14"><span class="bg-green"> '.$row_detail['pesan'].'';
        if ($row_detail['status_terbaca_photographer']=="terbaca") {
  ...
  public function add_pesan()
  {
    ...
    $cek_idlist_inbox= $this->MF_inbox->cek_idlist_inbox($custemer,$photographer)->result_array();
    $countdata=$this->MF_inbox->cek_idlist_inbox($custemer,$photographer)->num_rows();
    if ($countdata>0) {
